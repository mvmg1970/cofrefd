import type { ProcessarAtivoUseCase } from "../domain/usecases/processar-ativo";
...
export class NarrowApi {
  constructor(private readonly processarAtivo: ProcessarAtivoUseCase) {}

  async processar(requisicao: NarrowApiInput): Promise<NarrowApiOutput> {
    const result = await this.processarAtivo.executar({
